...
## 2024-08-04 - Arbitrary Permission Modification via Symlinks in os.chmod
**Vulnerability:** Calls to `os.chmod` were modifying permissions without checking if the target path was a symlink.
**Learning:** In Python, `os.chmod` follows symlinks by default. If an attacker controls the target path and replaces it with a symlink to an arbitrary file, the script will change the permissions of that arbitrary file (e.g. making `/etc/shadow` writable). This can lead to arbitrary permission modification and privilege escalation vulnerabilities. Note that `follow_symlinks=False` is not universally supported in Python depending on the OS platform.
**Prevention:** Always verify `if not path.is_symlink():` before calling `os.chmod` on existing paths, or avoid `os.chmod` on existing directories by securing them at creation time with `os.open(..., os.O_CREAT)` or `Path.mkdir(..., mode)`.
