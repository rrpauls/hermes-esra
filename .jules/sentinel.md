...
## 2026-07-28 - [Arbitrary Permission Modification via os.chmod Symlink Following]
**Vulnerability:** Across several Python tools, `os.chmod()` was called on directories or files after creation to enforce strict permissions (e.g., `0o700` or `0o600`). However, these operations did not check if the path was a symbolic link. Because `os.chmod` follows symlinks by default (and `follow_symlinks=False` is not universally supported), an attacker who could place a symlink at the expected path could cause the application to change the permissions of an arbitrary file or directory on the system.
**Learning:** Functions that enforce permissions (like `os.chmod`) can be tricked into modifying unintended files if they blindly follow symlinks created by a local attacker.
**Prevention:** Always verify that a path is not a symlink (e.g., using `if not path.is_symlink():`) before calling `os.chmod()`, or use atomic file creation flags that set the mode securely upon creation.
