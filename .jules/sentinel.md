...
## 2026-07-28 - [Arbitrary Permission Modification via Symlink in os.chmod]
**Vulnerability:** The codebase was calling `os.chmod()` on existing paths (such as directory setup steps) without verifying if the path was a symlink. Because `os.chmod()` follows symlinks by default in many Python environments, an attacker could plant a symlink at the expected path pointing to a sensitive file (e.g. `/etc/shadow`), causing the script to alter permissions on the attacker's target file rather than the expected directory.
**Learning:** `os.chmod` inherently follows symlinks unless specifically configured otherwise (which is platform dependent and not universally supported via `follow_symlinks=False`). Calling `os.chmod` on predictable, pre-existing paths in shared environments exposes the application to arbitrary permission modification vulnerabilities.
**Prevention:** Before calling `os.chmod` on an existing path, verify that the path is not a symlink by explicitly checking `not path.is_symlink()`. Alternatively, use `os.open` with `O_CREAT` and strict modes to atomically set file permissions upon creation, avoiding the need for `chmod` entirely.
