...
## 2026-07-28 - [Arbitrary Permission Modification via os.chmod Symlink Hijacking]
**Vulnerability:** Across multiple tools (`esra_paths.py`, `baseline_metrics.py`, `hermes_integration.py`, etc.), the codebase attempted to enforce secure POSIX directory permissions by calling `os.chmod(path, 0o700)` immediately after directory creation or checking existence. However, `os.chmod` natively follows symlinks. In a shared environment, an attacker could create a symlink at the expected path (e.g., `~/.hermes/config` pointing to `/etc/shadow`) before the script runs, causing `os.chmod` to modify the permissions of the attacker's chosen target file, leading to arbitrary permission modification and potential privilege escalation.
**Learning:** Functions that enforce permissions like `os.chmod` often follow symlinks by default. While Python 3.3+ supports `follow_symlinks=False` in `os.chmod()`, it is not universally supported across all platforms (notably raising `NotImplementedError` on Linux). Using `os.chmod` on predictable, user-controlled, or unverified paths without symlink checks introduces security vulnerabilities.
**Prevention:** Avoid calling `os.chmod` on existing paths without verifying that they are not symlinks. Always use `if not path.is_symlink():` before applying `os.chmod(path, mode)` to prevent symlink attacks and arbitrary permission modification.
